 ...
 * MediaRecorder 把录制的分辨率和比特率设置的高一些即可
 */
...
//        mHeight = mWidth = a.getInteger(R.styleable.MovieRecorderView_record_width, 640);//默认640
        mWidth = a.getInteger(R.styleable.MovieRecorderView_record_width, 640);//默认640
...
//        params.setPictureSize(480, 720);//拍照保存比率
    ...
    private void createRecordDir() {
        File sampleDir = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + "AAAAAAA/");
        if (!sampleDir.exists()) {
            sampleDir.mkdirs();
        }
        try {
...
//            LogUtil.e(LOG_TAG, recordFile.getAbsolutePath());
    ...
    private void initRecord() throws Exception {
        ...
        //    mediaRecorder.setVideoSize(1280, 1920);
